...
async function updateScore(input: UpdateScoreInput, actorId: string): Promise<Match> {
  ...
  if (!wasFinished) {
    ...
    if (league.seasonId && updatedMatch.phase === 'regular') {
      ...
      if (season && season.phase === 'regular') {
        ...
        if (isRegularSeasonComplete(leagueTeams.map((team) => team.id), allLeagueMatches)) {
          if (league.format === 'league-playoff') { await seasonService.setPhase(season.id, 'playoff'); await playoffService.generateTop8Playoff(league.id, season.id, actorId); await auditService.log(league.id, actorId, 'regular_season_finished', 'La fase regular terminó y se generó la liguilla Top 8.'); }
          else if (league.format === 'league-knockout') { await seasonService.setPhase(season.id, 'knockout'); await auditService.log(league.id, actorId, 'regular_season_finished', 'La fase regular terminó. La liga pasó a fase de eliminación.'); }
          else if (league.format === 'league') { await seasonService.finish(season.id); await auditService.log(league.id, actorId, 'season_finished', 'La fase regular terminó y la temporada fue finalizada.'); }
        }
      }
    }
  }
  ...
  if (!wasFinished && updatedMatch.phase === 'playoff' && updatedMatch.seasonId) {
    if (league.format === 'group-knockout') {
      ...
      if (updatedMatch.playoffRound === 'final' && updatedMatch.homeScore !== null && updatedMatch.awayScore !== null && updatedMatch.homeScore !== updatedMatch.awayScore) {
        const championTeamId = updatedMatch.homeScore > updatedMatch.awayScore ? updatedMatch.homeTeamId : updatedMatch.awayTeamId;
        await seasonService.finish(updatedMatch.seasonId, championTeamId);
        await auditService.log(league.id, actorId, 'champion_declared', `Campeón de la temporada: ${championTeamId}.`);
      }
    } else {
...
export const matchService = { getMatches, getMatchesByLeague, getMatchesByTeam, getMatchById, createMatch, updateScore, deleteMatch };
